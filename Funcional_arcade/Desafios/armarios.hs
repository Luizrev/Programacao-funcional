...
import Data.Either
import Data.Map
data Estado = Alugado | Livre deriving (Show, Eq)-- Estado == LockerState
type Codigo = String --Codigo == Code
type Armario = (Estado, Codigo)
type Armarios = Map.Map Int (Estado, Codigo) -- Armarios == LockerMap
...
pegarCodigo indice armarios =   
    case Map.lookup indice armarios of   
        Nothing -> Left $ "Armario " ++ show indice ++ " não existe"  
        Just (Estado, Codigo) -> if Estado /= Alugado   
                                then Right Codigo  
                                else Left $ "Letf " ++ show indice ++ " ja esta ocupado"  
